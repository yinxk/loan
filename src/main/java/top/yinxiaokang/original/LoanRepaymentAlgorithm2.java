 ...
 * 贷款还款算法
 */
public class LoanRepaymentAlgorithm2 {
    ...
    public final static int calInterestDays(Date dkffrq, Date ydkkrq) {
        if (dkffrq == null) {
            throw new ErrorException(ReturnEnumeration.Parameter_NOT_MATCH, "贷款发放日期不能为空");
        }
        if (ydkkrq == null) {
            throw new ErrorException(ReturnEnumeration.Parameter_NOT_MATCH, "约定扣款日期不能为空");
        }
     ...
     *
     * @param yearRate 没有经过小数处理的年利率(百分数)
     * @return 月利率  根据文档需要精确到%表示的小数点后6位, 也就是小数表示的8位
     */
     ...
     * @return 本期的逾期罚息
     */
    public final static BigDecimal calOverdueFx(BigDecimal yqbj, BigDecimal yqlx, BigDecimal dknlv, Date ywfsrq, Date ydkkrq) throws ErrorException {
     ...
     * @return 本期的逾期罚息
     */
    public final static BigDecimal calOverdueFx(BigDecimal yqbj, BigDecimal yqlx, BigDecimal dknlv, int yqts) throws ErrorException {
    ...
    public static BigDecimal calOverdueFxByYwfsrqOrOddAlgorithm(BigDecimal yqbj, BigDecimal yqlx, BigDecimal dknlv, Date dkffrq, int yqqc, Date ydkkrq, Date ywfsrq) {
        ...
        if (ywfsrq == null) {
            result = com.handge.housingfund.common.service.util.CommLoanAlgorithm.overdueFX(yqbj, yqlx, dknlv, dkffrq, yqqc, ydkkrq);
        } else {
